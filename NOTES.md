...
- **Cloud image storage.** Now uploads to Cloudinary (`backend/src/config/cloudinary.ts`, `backend/src/controllers/upload.controller.ts`) instead of local disk — requires `CLOUDINARY_CLOUD_NAME`/`CLOUDINARY_API_KEY`/`CLOUDINARY_API_SECRET` env vars. No image resizing/transformation params are applied yet beyond Cloudinary's defaults.
